...
module nand_read_output_datapath #(
    ...
    always @(posedge sys_clk or negedge sys_rst_n) begin
        if (!sys_rst_n) begin
            ...
            readout_ctrl_q <= readout_ctrl_i;
            readout_id_addr_q <= readout_id_addr_i;
            pb_rd_data_ready_o <= 1'b0;
                ...
                case (state_q)
                    ST_IDLE: begin
                        if (re_fall_i && (source == SOURCE_READ_ID)) begin
                            dq_out_o <= id_byte(readout_id_addr_i, read_ptr_q);
                            dq_oe_o <= 1'b1;
                            read_ptr_q <= read_ptr_q + 13'd1;
                        end else if (re_fall_i &&
                                     (source == SOURCE_READ_STATUS)) begin
                            dq_out_o <= nand_status_i;
                            dq_oe_o <= 1'b1;
                        end else if (page_read_start) begin
                            pb_rd_req_valid_o <= 1'b1;
                            pb_rd_addr_o <= read_ptr_q;
                            state_q <= ST_PB_REQ;
                        end
                    end
